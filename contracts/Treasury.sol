...
contract Treasury is ContractGuard, Epoch {
    ...
    function _getCashPrice(address oracle) internal view returns (uint256) {
      return IOracle(oracle).consult(cash, 1e18);
    }
    ...
    function setBondOracle(address _bondOracle) public onlyOperator {
        bondOracle = _bondOracle;
        emit BondOracleChanged(msg.sender, bondOracle);
    }

    function setSeigniorageBondOracle(address _seigniorageOracle) public onlyOperator {
        seigniorageOracle = _seigniorageOracle;
        emit SeigniorageBondOracleChanged(msg.sender, seigniorageOracle);
    }

    /* ========== MUTABLE FUNCTIONS ========== */

    function _updateCashPrice() internal {
      // FIXME: should probably check callable before calling update
      IOracle bo = IOracle(bondOracle);
      if (bo.callable()) {
        bo.update();
      }

      IOracle so = IOracle(seigniorageOracle);
      if (so.callable()) {
        so.update();
      }
    }
    ...
    event BondOracleChanged(address indexed operator, address newOracle);
    event SeigniorageBondOracleChanged(address indexed operator, address newOracle);
